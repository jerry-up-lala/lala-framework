...
public class ErrorHandle {
    ...
    private ExceptionInfoBO handle(Exception e, ApiLog apiLog) {
        ...
        // 错误配置
        CommonProperties.Error errorConfig = commonProperties.getError();
        if (errorConfig != null) {
            // 捕获异常
            Exception catchException = exceptionInfoBO.getCatchException();
            if (BooleanUtil.isTrue(errorConfig.getCatchPrint()) && catchException != null) {
                log.error("捕获异常", catchException);
            }

            // 业务异常
            ServiceException serviceException = exceptionInfoBO.getServiceException();
            if (BooleanUtil.isTrue(errorConfig.getServicePrint()) && serviceException != null) {
                log.error("业务异常", serviceException);
            }

            // 运行时异常
            if (BooleanUtil.isTrue(errorConfig.getRunTimePrint()) && catchException == null && serviceException == null) {
                log.error("运行时异常", e);
            }

            CommonProperties.Error.Mail errorMail = errorConfig.getMail();
            if (errorMail != null && BooleanUtil.isTrue(errorMail.getOpen())) {
                ...
                if (requestInfo != null) {
                    ...
                    exceptionEmailComponent.send(errorMail.getReceivers(), emailExceptionBO);
                }
            }
        }
    ...
    public ExceptionInfoBO loadExceptionInfoBO(Exception e) {
