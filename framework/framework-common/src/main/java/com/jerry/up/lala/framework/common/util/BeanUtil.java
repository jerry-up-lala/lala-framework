...
public class BeanUtil {
    ...
    public static <S, T> T toBean(S source, Class<T> targetType, String... targetTypeIgnoreProperties) {
        ...
        try {
            T target = cn.hutool.core.bean.BeanUtil.toBean(source, targetType, CopyOptions.create().setIgnoreProperties(targetTypeIgnoreProperties).setIgnoreError(true));
    ...
    public static <S, T> void copy(S source, T target, String... targetTypeIgnoreProperties) {
        ...
        try {
            cn.hutool.core.bean.BeanUtil.copyProperties(source, target, CopyOptions.create().setIgnoreProperties(targetTypeIgnoreProperties).setIgnoreError(true));
            beanFormat(source, target);
        } catch (Exception e) {
            log.error("转换属性异常", e);
        }
    ...
    public static <D> void beanFormatList(List<D> dataList) {
        if (dataList != null) {
            dataList.forEach(BeanUtil::beanFormat);
        }
    }
    ...
    private static String listToStr(Object sourceValue, String split) {
        if (CollUtil.isEmpty((List) sourceValue)) {
            return null;
        }
