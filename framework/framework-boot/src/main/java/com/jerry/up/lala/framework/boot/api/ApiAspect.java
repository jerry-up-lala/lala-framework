...
import com.jerry.up.lala.framework.boot.exception.ErrorHandle;
import com.jerry.up.lala.framework.boot.exception.ExceptionInfoBO;
import com.jerry.up.lala.framework.boot.properties.CommonProperties;
import com.jerry.up.lala.framework.boot.properties.LogProperties;
...
public class ApiAspect {
     ...
     * 控制层切入点
     */
    @Pointcut("execution(public * com.jerry.up.lala..*.*Ctrl.*(..)) && @annotation(com.jerry.up.lala.framework.boot.api.Api)")
            ...
            try {
                if (loadLog && apiLog != null) {
                    ...
                    if (BooleanUtil.isTrue(logProperties.getMq())) {
                        rocketMqTemplate.convertAndSend(logProperties.getMqTopic(), apiLog);
                    }
                }
            } catch (Exception e) {
